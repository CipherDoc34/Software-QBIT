#ifndef UUID2189407909680
#define UUID2189407909680
...
namespace ContinousMotionPipeline {
        ...
        #ifndef UUID2189114113376
#define UUID2189114113376
...
class Step0 {
    ...
    float offset[3] = {-19.58100000000f, -19.58100000000f, -19.54400000000f};
    float scale[3] = {0.02555910543f, 0.02555910543f, 0.02555910543f};
        ...
        #ifndef UUID2189396025984
#define UUID2189396025984
...
class Step1 {
    ...
    memcpy(x, queue, sizeof(float) * 312);

    
        memcpy(queue, queue + 93, sizeof(float) * 219);
    ...
    float queue[312] = {0};
        ...
        #ifndef UUID2189403585504
#define UUID2189403585504

/**
  * SpectralFeatures(num_inputs=3, window_length=104, order=2, num_features=20)
 */
class Step2 {
    public:
    ...
    for (uint16_t k = 0; k < 3; k++) {
        ...
        for (uint16_t i = k; i < 312; i += 3) {
            float xi = x[i];
            float abs_xi = abs(xi);
            float xi_prev2 = (i >= k + 6) ? x[i - 6] : xi;
        ...
        buffer[idx++] = cid_ce;
            ...
            float first_position_of_max = 312;
            float first_position_of_min = 312;
            float max_count = 0;
            float min_count = 0;
    ...
    memcpy(x, buffer, 60 * sizeof(float));
 ...
 *  > SpectralFeatures(num_inputs=3, window_length=104, order=2, num_features=20)
    ...
    class Pipeline {
        public:
            ...
            static const uint16_t NUM_OUTPUTS = 60;
            static const uint16_t WORKING_SIZE = 312;
            float X[312];
